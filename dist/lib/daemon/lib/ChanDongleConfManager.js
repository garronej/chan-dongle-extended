...
var _a = AmiCredential_1.AmiCredential.retrieve(), port = _a.port, host = _a.host, user = _a.user, secret = _a.secret;
...
var config = (function () {
    try {
        var out = iniExt_1.ini.parseStripWhitespace(fs_1.readFileSync(path, "utf8"));
...
function update() {
    var _this = this;
    return new Promise(function (resolve) { return fs_1.writeFile(path, iniExt_1.ini.stringify(config), { "encoding": "utf8", "flag": "w" }, function (error) { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (error)
...
function reloadChanDongle() {
    return new Promise(function callee(resolve) {
        var ami = new AstMan(port, host, user, secret, true);
        ami.once("close", function (hasError) {
            if (hasError)
                setTimeout(function () { return callee(resolve); }, 15000);
            else
                resolve();
        });
        ami.once("connect", function () {
            var actionId = ami.action({
                "action": "DongleReload",
                "When": "gracefully"
            }, function (error, res) {
                if (error)
